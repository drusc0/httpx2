...
class AsyncHTTP11ResponseNormalizingStream(AsyncNetworkStream):
    """
    ...
    def __init__(self, stream: AsyncNetworkStream, max_buffer_size: int) -> None:
        self._stream = stream
    ...
    async def read(self, max_bytes: int, timeout: float | None = None) -> bytes:
        ...
        while True:
            chunk = await self._stream.read(max_bytes, timeout)
            if not chunk:
                return chunk
...
class AsyncHTTP11Connection(AsyncConnectionInterface):
    ...
    def __init__(
        ...
        self._origin = origin
        self._network_stream = AsyncHTTP11ResponseNormalizingStream(stream, self.MAX_INCOMPLETE_EVENT_SIZE)
    ...
    async def handle_async_request(self, request: Request) -> Response:
        ...
        try:
            ...
            network_stream: AsyncNetworkStream = self._network_stream
